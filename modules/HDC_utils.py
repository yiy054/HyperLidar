...
class Model(nn.Module):
    ...
    def encode(self, x, mask=None, PERCENTAGE=None, is_wrong=None):
        ...
        if PERCENTAGE is not None:
            # # Pick by 50% top losses and 50% random: 
            num_req_samples = int(x.shape[0] * PERCENTAGE)
            num_wrongdata = num_req_samples // 2 # half from wrong data
            ...
            remaining_req_samples = num_req_samples - num_wrongdata
            if remaining_indices.numel() >= remaining_req_samples:
                random_fill_indices = remaining_indices[torch.randperm(remaining_indices.shape[0])[:remaining_req_samples]]
            else:
            ...
            # sorted_loss, sorted_indices = torch.sort(is_wrong, descending=True)
            # selected_indices = sorted_indices[:num_req_samples]  # pick top N
